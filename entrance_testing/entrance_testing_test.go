...
func TestGetTasks_EdgeCases(t *testing.T) {
	tests := []struct {
		name      string
		input     string
		user      *string
		...
		{
			name: "смешанные корректные и некорректные строки",
			input: `
TICKET-12345_Паша_Готово_2024-01-01
Некорректная строка 1
TICKET-12346_Иван_В работе_2024-01-02
TICKET-12347_Анна_Статус123_2024-01-03
Еще одна некорректная
TICKET-12348_Мария_Длинный статус_2024-01-04
`,
			user:      nil,
			status:    nil,
			wantCount: 4,
		...
		{
			name: "строки с лишними пробелами",
			input: `
  TICKET-12345_Паша_Готово_2024-01-01  
TICKET-12346_  Иван  _В работе_2024-01-02
TICKET-12347_Анна_  Статус  _2024-01-03
`,
			user:      nil,
			status:    nil,
			wantCount: 3,
		...
		{
			name: "фильтрация по несуществующему статусу",
			input: `
TICKET-12345_Паша_Готово_2024-01-01
TICKET-12346_Иван_В работе_2024-01-02
`,
			user:      nil,
			status:    stringPtr("Несуществующий статус"),
			wantCount: 0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GetTasks(tt.input, tt.user, tt.status)
			if len(got) != tt.wantCount {
				t.Errorf("GetTasks() count = %d, want %d", len(got), tt.wantCount)
			}
		})
	}
}
